...
const FriendCard: React.FC<FriendCardProps> = ({ friend, mode, requestId, onAction }) => {
  return (
    <motion.div
      layout
      ...
      className="relative bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10 overflow-hidden group"
    >
...
const EmptyState: React.FC<EmptyStateProps> = ({ icon, title, description }) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    className="col-span-full flex flex-col items-center justify-center text-center bg-white/5 rounded-xl p-12 border border-white/10"
  >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              className="flex-1 px-4 py-2 bg-gradient-to-r from-purple-500 to-blue-500 rounded-lg font-medium hover:shadow-lg transition-all"
            >
              Send Request
            </button>
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-white/5 rounded-lg font-medium hover:bg-white/10 transition-all"
            >
              Cancel
            </button>
          </div>
...
const FriendsPage: React.FC = () => {
  ...
  const refreshFriendsData = async (userId: string) => {
    try {
      // Cast the Promise.all result to the correct types
      const [friendsData, requestsData] = await Promise.all([
        getFriends(userId),
        getFriendRequests(userId),
      ]) as unknown as [Friend[], FriendRequest[]];
              ...
              <div className="w-full md:w-auto space-y-4">
                <div className="relative">
                  <input
                    ...
                    className="w-full md:w-80 px-5 py-3 bg-white/5 border border-white/10 rounded-full focus:outline-none focus:border-purple-500 transition-all"
                    value={searchQuery}
                ...
                <motion.button
                  ...
                  className="w-full md:w-auto px-6 py-3 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full font-medium flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-purple-500/20 transition-all"
                >
