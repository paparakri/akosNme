...
def calculate_hottest(df_clubs, df_reviews, location, limit=10):
    '''
    THIS NEEDS TO BE MADE DIFFERENT. (IF LOCATION IS athens instead of athens, greece FOR EXAMPLE)
    Possible solutions:
        1.Make sure there are only specific locations to select and then change the code a bit.
        2.Find the x,y coordinates of locations and calculate distance.

        1 seems easier but may turn out to be harder for specific situations whereas 2 seems more well estimated.
    ...
    club_reviews = pd.merge(local_clubs, df_reviews, left_on='_id', right_on='revieweye', how='left')

    # Calculate metrics for each club
    club_metrics = club_reviews.groupby('_id_x').agg({
        'revieweye': 'count',  # Count of reviews
        'createdAt': 'max',
        'displayName': 'first',
        'username': 'first'
    }).reset_index()
    ...
    #printing all hotness scores next to their club names
    #for index, row in club_metrics.iterrows():
    #    print(f"\nClub ID: {row['username']}, Review Count: {row['review_count']}, Rating: {row['rating']}, Recency Score: {row['recency_score']}, Hotness Score: {row['hotness_score']}\n")
